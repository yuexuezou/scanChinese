...
# 导入待测试模块
try:
    from ocr.engines.rapidocr_engine import RapidOCREngine, RAPIDOCR_AVAILABLE
except ImportError:
    RAPIDOCR_AVAILABLE = False


@unittest.skipIf(not RAPIDOCR_AVAILABLE, "RapidOCR不可用，跳过测试")
class TestRapidOCREngine(unittest.TestCase):
    """RapidOCR引擎测试类"""
    ...
    def setUp(self):
        ...
        # 模拟RapidOCR结果
        self.mock_result = (
            [[[10, 10], [90, 10], [90, 50], [10, 50]]],  # boxes
            [["测试文本"]],  # texts
            [[0.95]]  # scores
        )
    
    @patch('rapidocr_onnxruntime.RapidOCR')
    def test_init(self, mock_rapidocr):
        """测试初始化"""
        # 设置mock对象行为
        mock_rapidocr.return_value = MagicMock()
        
        # 初始化引擎
        engine = RapidOCREngine(use_angle_cls=True, gpu=False)
        
        # 验证初始化结果
        self.assertIsNotNone(engine)
        self.assertTrue(engine.use_angle_cls)
    ...
    def test_recognize(self, mock_rapidocr):
        ...
        mock_instance.return_value = self.mock_result
